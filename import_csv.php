...
const NOMESCRIPT	 = "Yetiforce Importer CSV";
const LOCK_FILE      = __DIR__ ."/importercsv.lock";   //File di lock per verificare se lo script è già in esecuzione
const PROFILE_DIR    = __DIR__ ."/profile/csv/";			    //Cartella che contiene i profili di esecuzione dello script
const ADMINLOG_DIR   = __DIR__ ."/log/csv/";                    //Cartella che contiene i log di amministrazione dello script
		...
		try{
			...
			switch (json_last_error()) {
		        ...
		        case JSON_ERROR_SYNTAX:
		            throw new Exception('Syntax error, malformed JSON - Controllare le virgole alla fine delle ultime righe. Controllare escape caratteri speciali');
		        break;
...
foreach( $profiles as $profile)
{
	...
	try{
		$importer= new CsvImporter($profile);
...
function adminReport($listLogProfile=null)
{
	try
	{
		...
		$mail->setFrom('fromemail@domain', NOMESCRIPT);
